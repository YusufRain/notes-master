...
Idempotent; latincedeki "eş(denk)" anlamına gelen "idem", ve ingilizcede "güç" anlamına gelen "potent", terimlerinin birleştirilmesinden oluşturulmuştur. Idempotent teriminin matematik ve bilişim dünyasında aşağıda anlatılan anlamı ile kullanılır.
...
Idempotent terimi genel olarak yazılım dünyasında fonskiyonlar için kullanılmaktadır.
...
Bu konu başlığı da okunması faydalı olacaktır: [file:"java.md" - title:"JSP ile single page application"](https://github.com/ysf0/notes/blob/master/notes/java.md#jsp-ile-single-page-application)
...
controller içinde async bir thread üzerinden (timeout, ajax call, cordova pugin'in return callback'leri gibi) bir dinamic değer değiştirildiğinde, controller'ın main thred'inden sonra çalışırsa controller'ın haberi olmuyor. bu sebeple $scope.apply() metodu geliştirilmiştir. bu metodu çağrıldığında tüm dinamic değerler angular tarafından tekrar kontrol edilmektedir. örneğin; ajax-callback thred'in sonunda apply() methodu çağrılabilir.
